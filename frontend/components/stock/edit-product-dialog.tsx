...
export function EditProductDialog({ open, onOpenChange, product, onEdit }: EditProductDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (Number.isInteger(asSupplierId) && asSupplierId > 0)
            payload.supplier_id = asSupplierId;
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (Array.isArray(data)) {
                            ...
                            if (found && found.id)
                                payload.supplier_id = Number(found.id);
                        }
                    }
                }
                catch (e) {
        ...
        if (Number.isInteger(asCategoryId) && asCategoryId > 0)
            payload.category_id = asCategoryId;
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (Array.isArray(data)) {
                            ...
                            if (found && found.id)
                                payload.category_id = Number(found.id);
                        }
                    }
                }
                catch (e) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="edit-name" value={formData.name} onChange={(e) => handleChange("name", e.target.value)} className="col-span-3" required/>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="edit-sku" className="text-right">
                SKU
              </Label>
              <Input id="edit-sku" value={formData.sku} onChange={(e) => handleChange("sku", e.target.value)} className="col-span-3" required/>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
            ...
            return (<SelectItem key={key} value={value}>
                        {c.name}
                      </SelectItem>);
        })}
                </SelectContent>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="edit-quantity" type="number" value={formData.quantity} onChange={(e) => handleChange("quantity", e.target.value)} className="col-span-3" required/>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="edit-price" className="text-right">
                Price
              </Label>
              <Input id="edit-price" type="number" step="0.01" value={formData.price} onChange={(e) => handleChange("price", e.target.value)} className="col-span-3" required/>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="edit-threshold" className="text-right">
                Low Stock
              </Label>
              <Input id="edit-threshold" type="number" value={formData.lowStockThreshold} onChange={(e) => handleChange("lowStockThreshold", e.target.value)} className="col-span-3" required/>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
