...
export function Toaster() {
    return (<ToastPrimitive.Viewport className="fixed bottom-6 right-6 z-50 flex flex-col gap-2"/>);
}
export function Toast({ title, description, variant = 'default', open, onOpenChange }: {
    ...
    return (<ToastPrimitive.Root className={cn('rounded-md shadow-md p-3 w-80', variant === 'success'
            ? 'bg-green-600/10 backdrop-blur-sm border border-green-400/30'
            : variant === 'error'
                ? 'bg-red-600/10 backdrop-blur-sm border border-red-400/30'
                : 'bg-card/90')} open={open} onOpenChange={onOpenChange}>
