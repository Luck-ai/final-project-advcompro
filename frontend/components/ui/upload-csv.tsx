...
export default function UploadCSV({ url, label = "Upload CSV", accept = ".csv" }: UploadCSVProps) {
    ...
    return (<div className="space-y-2">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <input id="upload-csv-input" type="file" accept={accept} onChange={onFileChange} className="sr-only"/>

        <label htmlFor="upload-csv-input" className="inline-flex items-center px-3 py-2 rounded-md border bg-background hover:bg-accent/50 text-sm cursor-pointer">
          Choose File
        </label>
      ...
      {results ? (<div className="mt-2 max-h-56 overflow-auto bg-muted p-2 rounded">
          <table className="w-full text-sm">
            ...
            <tbody>
              {results.map((r: any, idx: number) => (<tr key={idx} className="border-t">
